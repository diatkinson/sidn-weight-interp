...
def evaluate(args, model, loader) -> list[float]:
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(loader, desc="Evaluating"):
            ...
            labels: Float[Tensor, "batch 1000"] = labels.to(args.device)
            ...
            for k in range(1, 1001):
                ...
                labels_in_top_k = torch.any(top_k == labels.unsqueeze(1), dim=1)
                correct_counts[k - 1] += labels_in_top_k.sum().item()

            total_count += labels.size(0)

    # Calculate percentages
    accs = [100 * count / total_count for count in correct_counts]
...
if __name__ == "__main__":
    ...
    random_acc = evaluate(args, RandomBaseline(1000), test_loader)
    print_accuracies("Random", random_acc)

    acc = evaluate(args, model, test_loader)
    print_accuracies("Standard", acc)

    print("Evaluating scrambled")

    scrambled_acc = {}
    scramble_counts = [1, 5, 10, 20, 50, 100]
    for scramble_count in scramble_counts:
        print(f"Scrambling {scramble_count} times")
        _, _, test_loader = get_dataloaders(
            model_dir,
            batch_size=batch_size,
            train_workers=10,
        ...
        scrambled_acc[scramble_count] = evaluate(args, model, test_loader)
    # print_accuracies("Scrambled", scrambled_acc)
    ...
    def create_plot(plot_name, xscale="linear"):
        ...
        plt.plot(k_values, acc, label="Standard evaluation")
        for scramble_count in scramble_counts:
            plt.plot(k_values, scrambled_acc[scramble_count], label=f"Scrambled ({scramble_count}) evaluation")
        plt.plot(k_values, random_acc, label="Random")
        plt.xlabel("k")
        plt.ylabel("Accuracy (%)")
    ...
    # Normal plot
    create_plot("scrambled_top_k_accuracy_plot.png")

    # Log plot
    create_plot("scrambled_top_k_accuracy_log_plot.png", xscale="log")
